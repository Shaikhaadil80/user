...
class Setting {
  ...
  factory Setting.fromJSON(Map<String, dynamic> jsonMap) {
    List<String> homeSetting=[];
    for (int _i = 1; _i <= 12; _i++) {
       homeSetting.add(jsonMap['home_section_' + _i.toString()] != null
            ? jsonMap['home_section_' + _i.toString()]
            : 'empty');}
      return Setting(
        appName : jsonMap['app_name'] .toString(),
      ...
      mobileLanguage : ValueNotifier(Locale(jsonMap['mobile_language'] ?? "en", '')),
